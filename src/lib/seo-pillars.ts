  ...
  },
  {
    ...
    actions: [
      ...
      },
      {
        ...
        blurb:
          "Rich, valid JSON-LD (@graph) — crawls the live page, auto-detects the market, cross-links entities.",
        // Crawl the live page first so the schema is built from the real NAP,
        // hours, services, FAQs and any existing JSON-LD — not just pasted text.
        tools: ["crawl-page"],
        fields: [
          {
            key: "schemaType",
            ...
            defaultValue: "Auto / @graph (recommended)",
            helpText:
              "Auto lets SEO Claude pick the most specific type(s) and combine them in one @graph (business + WebPage + BreadcrumbList + FAQPage), cross-linked by @id.",
          },
          {
            key: "market",
            ...
            defaultValue: "Auto-detect from page",
            helpText:
              "Sets the address country, language tag and currency in the schema. Auto-detect reads the page's OWN address, phone, TLD and copy language — it never uses the client's rank-tracking geo.",
          },
          URL_FIELD,
          {
            key: "pageContent",
            label: "Source content / facts (optional)",
            type: "textarea",
            required: false,
            rows: 6,
            placeholder:
              "Optional when a URL is given — we crawl the page automatically. Paste extra facts (opening hours, prices, services, FAQs, social profiles) to enrich the schema further.",
          },
        ],
      },
      {
