    ...
    <div className="animate-fade-up flex flex-wrap items-center gap-3">
      <div className="relative min-w-0 flex-1 sm:max-w-[440px]">
        ...
        <input
          ...
          className="w-full rounded-full border border-white/12 bg-white/[0.04] py-2.5 pl-10 pr-16 text-sm text-white/85 outline-none transition placeholder:text-white/30 focus:border-[color:var(--brand-purple)]/50 focus:bg-white/[0.07] focus:ring-2 focus:ring-[color:var(--brand-purple)]/15 [&::-webkit-search-cancel-button]:hidden"
        />
        {value ? (
          <button
            type="button"
            ...
            className="absolute right-3 top-1/2 -translate-y-1/2 rounded-full p-1 text-white/40 transition hover:bg-white/[0.08] hover:text-white"
          >
          ...
          <kbd
            aria-hidden
            className="absolute right-3 top-1/2 -translate-y-1/2 rounded border border-white/12 px-1.5 py-0.5 text-[10px] font-medium text-white/30"
          >
            /
          </kbd>
        )}
      </div>
      <p className="readout text-white/30">
        {value ? `${found} de ${total}` : `${total} ferramentas`}
      </p>
    </div>
  );
}
