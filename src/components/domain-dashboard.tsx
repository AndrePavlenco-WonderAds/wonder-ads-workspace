...
function AiVisibilityPanel({ mentions }: { mentions: LlmMentions }) {
  const hasGlobal = !!mentions.global;
  return (
    <div className="relative overflow-hidden rounded-2xl border border-violet-400/25 bg-gradient-to-br from-violet-500/[0.06] via-white/[0.02] to-transparent p-5">
      ...
      <p className="mb-4 text-[11.5px] leading-snug text-white/55">
        How often this domain appears as a source in answers generated by
        Google AI (AI Overview, AI Mode, Gemini-in-search) and ChatGPT.{" "}
        <strong className="text-white/75">Local</strong> = the client&apos;s
        own market &amp; language — the audience that actually buys.
        {hasGlobal && (
          <>
            {" "}
            <strong className="text-white/75">Global English</strong> = US/en
            baseline — broader signal since the LLM corpus is densest in
            English (a non-EN domain can still earn English AI citations).
          </>
        )}
      </p>
      ...
      <MentionsWindowCard
        title="Local market"
        subtitle="Audience that buys from this client"
        window={mentions.primary}
        accent="violet"
      />
      ...
      {hasGlobal && (
        <div className="mt-4">
          <MentionsWindowCard
            title="Global English baseline"
            subtitle="Wider US/en signal — useful when local is sparse"
            window={mentions.global!}
            accent="sky"
          />
        </div>
      )}
...
function MentionsWindowCard({
  ...
  title: string;
  subtitle: string;
  window: LlmMentionsWindow;
  accent: "violet" | "sky";
}) {
  const ringClass =
    accent === "violet" ? "border-violet-400/20" : "border-sky-400/20";
  const badgeClass =
    accent === "violet"
  ...
  return (
    <div className={`rounded-xl border ${ringClass} bg-black/15 p-4`}>
      <header className="mb-3 flex flex-wrap items-baseline gap-2">
        <h4 className="text-[12px] font-semibold tracking-tight text-white">
          {title}
        </h4>
        <span
          className={`rounded-full border px-2 py-0.5 text-[10px] uppercase tracking-[0.13em] ${badgeClass}`}
        >
          {w.label}
        </span>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <Stat
          ...
          value={w.totalMentions}
          sublabel="Times this domain was cited in LLM answers"
        />
        <Stat
          icon={<Activity className="h-3.5 w-3.5" />}
          accent={accent}
          label="AI search volume"
          value={fmtNum(w.aiSearchVolume)}
          sublabel="Queries that could trigger a mention"
        />
        {w.perPlatform.map((p) => (
          <Stat
            key={p.platform}
            ...
            label={`${p.platform === "google" ? "Google AI" : "ChatGPT"} mentions`}
            value={p.mentions}
      ...
      {(w.topCitedPages.length > 0 || w.coCitedDomains.length > 0) && (
        <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
          {w.topCitedPages.length > 0 && (
            <div className="rounded-xl border border-white/8 bg-black/20 p-3">
              <h5 className="mb-1 flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-[0.13em] text-white/65">
                <Award className="h-3 w-3 text-violet-300" />
                Top cited pages from this domain
              </h5>
              <p className="mb-2 text-[10.5px] text-white/40">
                Pages on this site that LLMs link to most when answering queries
              </p>
              <ul className="space-y-1.5">
                {w.topCitedPages.slice(0, 8).map((p, i) => (
                  <li
                    key={i}
                  ...
                  >
                    ...
                    <span
                      className={`shrink-0 rounded px-1.5 py-0.5 text-[10px] font-medium ${pageBadge}`}
                    >
                      {p.mentions}×
                    </span>
                  </li>
          ...
          {w.coCitedDomains.length > 0 && (
            <div className="rounded-xl border border-white/8 bg-black/20 p-3">
              ...
              <p className="mb-2 text-[10.5px] text-white/40">
                Competitors + sources that appear alongside us in LLM answers
              </p>
              <ul className="space-y-1.5">
                {w.coCitedDomains.slice(0, 10).map((d, i) => (
                  <li
                    key={i}
                    className="flex items-baseline gap-2 text-[11.5px]"
                  >
      ...
      {w.totalMentions === 0 && (
        <p className="mt-3 text-[11px] text-white/40">
          No mentions detected in this window. For non-English markets, the
          DataforSEO LLM corpus may be sparse — check the Global English
          baseline above.
        </p>
      )}
    </div>
